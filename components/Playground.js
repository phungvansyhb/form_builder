    ...
    function renderInput(config, index) {
        return (
            <div className={`${colWidth(config)} flex flex-col gap-2 `} key={index}>
                <label htmlFor={config.name} className='w-20'>{config.label} :</label>
                <input {...register(config.name, {required: true})} disabled={config.disable}
                       defaultValue={config.defaultValue} className='input flex-grow'/>
                {formState.errors[config.name]?.type === 'required' && `${config.name} is required`}
            </div>
                                    ...
                                    <div className='flex gap-4'>
                                        <form onSubmit={handleSubmit(onSubmit)}>
                                            <div className="mt-2 grid grid-cols-12 gap-4">
                                                {
                                                    inputContext.inputConfigs.map((config, index) => renderInput(config, index))
                                                }
                                            </div>
                                        </form>
                                        <div className='w-1/2'>
